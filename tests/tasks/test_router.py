...
CREATE_TASK_DATA = {"title": "New API Task", "description": "Create via API", "status": TaskStatus.NEW}
UPDATE_TASK_DATA = {"id": 101, "title": "Updated Title", "status": TaskStatus.IN_PROGRESS}
...
async def test_get_task_success(client: TestClient, mock_task_service: MagicMock):
    ...
    task_id = 101
    mock_task_service.get_task.return_value = TASK_RESPONSE_EXPECTED # Return data matching response model
...
async def test_get_task_not_found(client: TestClient, mock_task_service: MagicMock):
    ...
    mock_task_service.get_task.side_effect = NotFoundException("Task not found")
...
async def test_list_tasks_success(client: TestClient, mock_task_service: MagicMock):
    ...
    mock_task_service.get_tasks.assert_awaited_once_with(page=1, elements_per_page=5, status=None)
...
async def test_list_tasks_with_status(client: TestClient, mock_task_service: MagicMock):
    ...
    assert response.status_code == status.HTTP_200_OK
    assert response.json() == [TASK_RESPONSE_EXPECTED]
    mock_task_service.get_tasks.assert_awaited_once_with(page=1, elements_per_page=10, status=test_status.value)
...
async def test_list_user_tasks_success(client: TestClient, mock_task_service: MagicMock):
    ...
    assert response.status_code == status.HTTP_200_OK
    assert response.json() == TASK_LIST_RESPONSE_EXPECTED
    mock_task_service.get_user_tasks.assert_awaited_once_with(user_id=user_id_to_test, page=2, elements_per_page=10)
...
async def test_update_task_success(client: TestClient, mock_task_service: MagicMock):
    ...
    assert call_kwargs['schema'].model_dump(exclude_unset=True) == UPDATE_TASK_DATA
    assert call_kwargs['user_id'] == TEST_USER_ID
...
async def test_update_task_not_found(client: TestClient, mock_task_service: MagicMock):
    ...
    from src.base.exceptions import NotFoundException
    mock_task_service.update_task.side_effect = NotFoundException("Task not found")
...
async def test_update_task_unauthorized(client: TestClient, mock_task_service: MagicMock):
    ...
    mock_task_service.update_task.side_effect = UnAuthorizedException("Unauthorized")
...
async def test_delete_task_success(client: TestClient, mock_task_service: MagicMock):
    ...
    assert response.status_code == status.HTTP_200_OK
    mock_task_service.delete_task.assert_awaited_once_with(task_id=task_id_to_delete, user_id=TEST_USER_ID)
...
async def test_delete_task_not_found(client: TestClient, mock_task_service: MagicMock):
    ...
    from src.base.exceptions import NotFoundException
    mock_task_service.delete_task.side_effect = NotFoundException("Task not found")
...
async def test_delete_task_unauthorized(client: TestClient, mock_task_service: MagicMock):
    ...
    task_id_to_delete = 101
    from src.base.exceptions import UnAuthorizedException
    mock_task_service.delete_task.side_effect = UnAuthorizedException("Unauthorized")
